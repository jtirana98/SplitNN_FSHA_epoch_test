...
class leakageFSHA:
    ...
    def __init__(self, xpriv, id_setup, batch_size, hparams, attack_model):
        ...
        self.logger = attack_model.logger
    ...
    def train_step(self, x_private, label_private):
        ...
        X_recovered = self.attack_model.attack_other(x_private, self.f)
        mse_score = FSHA.distance_data(x_private, X_recovered)
        
        accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
        return  D_loss, accuracy, mse_score
    ...
    def __call__(self, iterations, log_frequency=500, verbose=False, progress_bar=True):
        ...
        for x_private, label_private in iterator:
            ...
            if i == 0:
                ...
                VAL_mse += log[2] / log_frequency

            if  i % log_frequency == 0:
                LOG[j] = log

                if verbose:
                    self.logger.info("log--%02d%%-%07d] validation: %0.4f acc: %0.4f MSE: %0.4f" % ( int(i/iterations*100) ,i, VAL, VAL_A, VAL_mse) )
